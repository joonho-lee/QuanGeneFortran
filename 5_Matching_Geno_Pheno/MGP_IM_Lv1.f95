...
integer::NOA=0,ANO=0,NOM=0,MNO=0,NOR=0,RNO=0,NOTR=0,TRNO=0,FENO=0,NOFE=0,MNODPR=0,TNODPR=0 !numbers and counting variables
integer,allocatable::G012DS(:,:),TG012DS(:,:),FEDS(:,:),MNOMPR(:),MNOOPR(:),MNOPRBT(:),TNOMPR(:),TNOOPR(:),TNOPRBT(:) !data storage
real::MV
...
character(len=3),allocatable::NAOTR(:),NAOFE(:)
character(len=50)::MN
character(len=8)::OFT,NOAF,NOMF,NOFEF,NOTRF
...
allocate(IDG(NOA))
read (99,*) NOFEF
print *, "number of fixed effects", NOFE
read (99,*) NOTRF
...
WF3='            '
write(WF3,fmt='(a16)') '('//adjustr(NOFEF)//'i10   )'
print *, "writing format for fixed effect file is : ",WF3
WF4='            '
write(WF4,fmt='(a16)') '('//adjustr(NOTRF)//'f10.3 )'
print *, "writing format for observation for trait is : ",WF4
WF5='                           '
...
allocate(FEDS(NOR,NOFE),PRDS(NOR,NOTR))
allocate(IDP(NOR))
do RNO=1,NOR
  read(97,*) IDP(RNO),FEDS(RNO,:),PRDS(RNO,:)
enddo
