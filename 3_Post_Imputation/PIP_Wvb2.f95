...
! Version : Windows 1.0 (2012-06-25)
...
integer::NOAC=0,SMNBC=1,NOA=0,ANO=0,NOM=0,MNO=0,NOUM=0 !number of chromosomes, animals and markers & chromosome, animal and marekr number
integer::BC(4)
integer::GC(10)
integer::CMPBC=0,EXMPBC=0,MDG(4)
integer::NODBAM1=0,NODBAM2=0,NODBAM3=0,NODBAM4=0,NODBAM5=0
integer::NOSMWMAF1=0,NOSMWMAF2=0,NOSMWMAF3=0,NOSMWMAF4=0,NOSMWMAF5=0
...
integer,allocatable::G012DS(:,:),CG101DS(:,:),NOSMBC(:),NOUMBC(:),IOM(:,:),ON(:) !data input
...
character(len=30),allocatable::ID(:),DID(:)
character(len=38),allocatable::IDFNS(:)
character(len=50),allocatable::MN(:)

!reading parameters
print *, "***********************************************************************************************************************" 
print *, "PIP_WvB : Reformation of Imputation Results & Basic Statistics Analysis Windows version BEAGLE"
...
if(NOTA.ne.NOA) then
  read (99,*) NOFT
  print *, "previous output tag ", NOFT
  read (99,*) ADD
...
allocate(NOSMBC(NOAC),NOUMBC(NOAC),IDFNS(NOAC),GDS1(NOTA*2))
NOUMBC=0
do i=1,NOAC
...
write(92,fmt='(a5,a10,28a15)') "CHNO","NO.SNP","AMAF","SDMAF","MINMAF","MAXMAF","AIBC","SDIBC","MINIBC","MAXIBC","ACSOHWT",&
& "SDCSOHWT","MINCSOHWT","MAXCSOHWT","APIC","SDPIC","MINPIC","MAXPIC","AHE","SDHE","MINHE","MAXHE","AEH","SDEH","MINEH","MAXEH"&
...
write(84,fmt='(a50,a10,2a15)') "Marker Name","MarkerNO","chromosome","position"
ODFNIP=ODD//"Deleted_Markers"//OFT//".out"
ODFNIP=adjustl(ODFNIP)
open(83,file=ODFNIP,status='unknown')
write(83,fmt='(a50,3a15)') "Marker Name","FOMIG","FOHE","CSOHWT"
if(NOTA.ne.NOA) then
  DDD=adjustr(DDD)
  ADD=adjustr(ADD)
  ADFNIP=ADD//ADFN
  ...
  write(85,fmt='(a30,2a10)') "Animal ID    ","Animal NO.","Animal NNO."
  read(87,*) 
  do q=1,NOTA
    read(87,*) ID(q),ON(q)
  enddo
...
DBAMBC=0;CMPBC=0;EXMPBC=0
  do j=1, NOSMBC(i)
    ...
    if(FOMIG<IOL(2).or.CSOHWT>IOL(3).or.FOHE.eq.1) then 
      print *, "Marker",MNO,"looks unusual",FOMIG,FOHE,CSOHWT
      write(83,fmt='(a50,3f15.5)') MN(MNO),FOMIG,FOHE,CSOHWT
      NOUMBC(i)=NOUMBC(i)+1
      NOUM=NOUM+1 
    else
      ...
      write(84,fmt='(a50,i10,2i15)') MN(MNO),IOM(MNO,1),IOM(MNO,2),IOM(MNO,3)
    endif
  enddo
  AOMAFBC=SOMAFBC/real(NOSMBC(i)-NOUMBC(i))
  AOIBCBC=SOIBCBC/real(NOSMBC(i)-NOUMBC(i))
  AOCSOHWTBC=SOCSOHWTBC/real(NOSMBC(i)-NOUMBC(i))
  AOPICBC=SOPICBC/real(NOSMBC(i)-NOUMBC(i))
  AOHEBC=SOHEBC/real(NOSMBC(i)-NOUMBC(i))
  AOEHBC=SOEHBC/real(NOSMBC(i)-NOUMBC(i))
  AODBAMBC=SODBAMBC/real(NOSMBC(i)-NOUMBC(i)-1)
  SDOMAFBC=(SSOMAFBC-SOMAFBC**2/real(NOSMBC(i)-NOUMBC(i)))/real(NOSMBC(i)-NOUMBC(i))
  SDOIBCBC=(SSOIBCBC-SOIBCBC**2/real(NOSMBC(i)-NOUMBC(i)))/real(NOSMBC(i)-NOUMBC(i))
  SDOCSOHWTBC=(SSOCSOHWTBC-SOCSOHWTBC**2/real(NOSMBC(i)-NOUMBC(i)))/real(NOSMBC(i)-NOUMBC(i))
  SDOPICBC=(SSOPICBC-SOPICBC**2/real(NOSMBC(i)-NOUMBC(i)))/real(NOSMBC(i)-NOUMBC(i))
  SDOHEBC=(SSOHEBC-SOHEBC**2/real(NOSMBC(i)-NOUMBC(i)))/real(NOSMBC(i)-NOUMBC(i))
  SDOEHBC=(SSOEHBC-SOEHBC**2/real(NOSMBC(i)-NOUMBC(i)))/real(NOSMBC(i)-NOUMBC(i))
  SDODBAMBC=(SSODBAMBC-SODBAMBC**2/real(NOSMBC(i)-NOUMBC(i)-1))/real(NOSMBC(i)-NOUMBC(i)-1)
  write(92,fmt='(i5,i10,28f15.5)') i,NOSMBC(i)-NOUMBC(i),AOMAFBC,SDOMAFBC,MINOMAFBC,MAXOMAFBC,AOIBCBC,SDOIBCBC,MINOIBCBC,&
  MAXOIBCBC,AOCSOHWTBC,SDOCSOHWTBC,MINOCSOHWTBC,MAXOCSOHWTBC,AOPICBC,SDOPICBC,MINOPICBC,MAXOPICBC,AOHEBC,SDOHEBC,&
...
write(92,fmt='(a5,i10,28f15.5)') "Tot",NOM,AOTMAF,SDOTMAF,MINOTMAF,MAXOTMAF,AOTIBC,SDOTIBC,MINOTIBC,MAXOTIBC,&
...
&AOTEH,SDOTEH,MINOTEH,MAXOTEH,AOTDBAM,SDOTDBAM,MINOTDBAM,MAXOTDBAM
    ...
    write(88,fmt='(a120,i15)') "0.2 <= minor allele frequency < 0.3  :  ", NOSMWMAF3
