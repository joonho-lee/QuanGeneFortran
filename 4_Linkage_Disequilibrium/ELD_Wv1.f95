...
! Version : Windows 1.0 (2012-03-26)
...
!reading parameters
print *, "***********************************************************************************************************************" 
print *, "ELD_Lv1 : Extent of Linkage Disequilibrium Windows version 1.0"
print *, "Developed by Joonho Lee : zoonolee@gmail.com, +82-10-3408-2895"
print *, "Data form is the results of BEAGLE and of converted genotype 0,1,2"
...
SLDBC=0;ALDBC=0;SALDBC=0;AALDBC=0
  do MNO1=SEMNBC(CNO,1), SEMNBC(CNO,2)
    ...
    if(MNO2-MNO1.eq.1) then 
      ...
      do NEJ=1, NONEG
        if(HG.gt.NEG(NEJ,2).and.HG.le.NEG(NEJ,1)) then
          CNEG(NEJ)=CNEG(NEJ)+1
...
do NEJ=1, NONEG
  ...
  EPS=(1/((NEG(NEJ,1)+NEG(NEJ,2))/50))*((1/ANEG(NEJ))-1.0)
